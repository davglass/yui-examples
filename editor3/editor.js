...
YAHOO.widget.Editor = function(elm, userConfig) {
    ...
    this.config = {
        ...
        toolbarId: 'yuiEditor_toolbar_' + YAHOO.widget.Editor_Core.instances.length
    };
...
YAHOO.widget.Editor.prototype = {
    ...
    _assignListeners: function() {
       ...
       debug('DropDown1: ' + dd1);
        ...
        for (i in this.config.events) {
            YAHOO.util.Event.addListener(this._doc(), this.config.events[i], this._nodeTest, this, true);
        }
        var imgs = this._doc().getElementsByTagName('img');
        console.log(imgs);
        YAHOO.util.Event.on(imgs, 'click', function() {
            console.log('imgs click event: ' + this);
            YAHOO.util.Dom.setStyle(this, 'border', '1px solid black');
        });
        for (var i = 0; i < imgs.length; i++) {
            var tmp = new YAHOO.util.DD(imgs[i]);
            tmp.startDrag = function() {
                console.log('drag drop startDrag');
            }
            console.log(tmp);
        }
    },
    _nodeTest: function(ev) {
    ...
    */
    _createToolbar: function() {
        this.toolbar.onToolClick.subscribe(this._execCommand);
        this.toolbar.render();
        /*
    ...
    */
    _execCommand: function(ev, action, value) {
        if (ev.toLowerCase() == 'ontoolclick') {
            debug('_execCommand::Event: ' + ev);
            action = action[0].event;
            debug('_execCommand::Action: ' + action);
            debug('scope: ' + this);
        } else if (ev) {
            debug('_execCommand::Event: ' + ev);
            var tar = YAHOO.util.Event.getTarget(ev);
